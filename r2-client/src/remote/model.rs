use openssl_utils::{CryptoErr, aead::{AeadKey, SealedSecretBox, UnsealedSecretBox}, assymetric_secret::{KeySealer, KeyUnsealer, SealedAeadKey}};
use crate::{DocCollaborator, model::{Commit, UnverifiedCommit, Me}};
...
impl RemoteCollaborator {
	pub fn from_doc_collaborator(collaborator: &DocCollaborator, key: &DocumentKey) -> Result<Self, CryptoErr> {
...
impl CipheredCommit {
	pub fn cipher(commit: &Commit, key: &DocumentKey, nonce_src: &dyn CipheredCommitNonceSource) -> Result<CipheredCommit, Error> {
	...
	pub fn decipher(self, key: &DocumentKey) -> Result<UnverifiedCommit, Error> {
		...
		assert_eq!(commit.id, self.id, "ID mismatch between what you thought you were getting and what the server sent");
