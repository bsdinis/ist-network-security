...
pub enum CommitDomainError {
    ...
    #[error("Commit does not have a valid signature: {:?}", .0)]
    InvalidSignature(#[source] CryptoErr),
...
impl UnverifiedCommit {
    ...
    pub fn verify(self, author: &CommitAuthor) -> Result<Commit, CommitDomainError> {
        assert_eq!(
        ...
        if self.id != generated_id {
            return Err(CommitDomainError::BadlyGeneratedId {
    ...
    fn gen_id(&self) -> Result<String, CommitDomainError> {
        let bytes = self.bytes(false);
        let digest = hash(ID_DIGEST_ALGO.to_owned(), &bytes)
            .map_err(|e| CommitDomainError::IdGenerationFailed(e))?
            .to_vec();
...
impl CommitBuilder {
    ...
    /// in debug builds.
    pub fn author(self, me: &Me) -> Result<Commit, CommitDomainError> {
        ...
        commit.signature = me
            .sign(&bytes)
            .map_err(|e| CommitDomainError::SigningFailed(e))?;
