...
impl UnverifiedCommit {
    ...
    pub fn verify(self, author: &CommitSigner) -> Result<Commit, Error> {
        assert_eq!(
...
impl CommitBuilder {
    ...
    /// in debug builds.
    pub fn author(self, author: &CommitSigner) -> Result<Commit, Error> {
...
mod test {
    ...
    fn verify_ok() {
        ...
        let commit1 = ucommit1.clone().verify(&*COMMIT_SIGNER_B).unwrap();
        assert_eq!(ucommit1.id, commit1.id);
        assert_eq!(ucommit1.author_id, commit1.author_id);
        assert_eq!(ucommit1.ts, commit1.ts);
        assert_eq!(ucommit1.message, commit1.message);
    ...
    fn author_ok() {
        ...
        assert_eq!(commit0.author_id, COMMIT_SIGNER_B.id);
        ...
        assert_eq!(commit1.author_id, COMMIT_SIGNER_A.id);
        ...
        macro_rules! test {
            ($field:ident) => {
                ...
                fn $field() {
                    ...
                    assert!(
                        ucommit0.verify(&*COMMIT_SIGNER_A_VERIFY_ONLY).is_err(),
                        "verified commit with bad signature"
                    );
                    assert!(
                        ucommit1.verify(&*COMMIT_SIGNER_B).is_err(),
                        "verified commit with bad signature"
                    );
                }
            };
    ...
    fn verify_panic_author_mismatch() {
        ...
        let _ = ucommit0.verify(&*COMMIT_SIGNER_B_VERIFY_ONLY); // should be B
        let _ = ucommit1.verify(&*COMMIT_SIGNER_A); // should be A
