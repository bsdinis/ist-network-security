...
use super::repo::{RepoStorage, RepoStorageSharedGuard, RepoStorageExclusiveGuard};
...
impl Drop for FilesystemRepoStorageSharedGuard {
	fn drop(&mut self) {
		self.0.lock_file.unlock().expect("couldn't unlock repo (previously had shared lock)");
	}
}
		...
		impl RepoStorageSharedGuard<FilesystemRepoStorage> for $typename {
			...
			fn unlock(mut self) -> FilesystemRepoStorage {
				let cloned = FilesystemRepoStorage {
					...
					lock_file: self.0.lock_file.try_clone().expect("File::try_clone shouldn't error really"),
				};
