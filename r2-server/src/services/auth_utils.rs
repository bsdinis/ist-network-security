...
pub async fn authenticate<T>(req: &Request<T>) -> Result<Vec<u8>, Status> {
    ...
    id.ok_or(Status::internal(
        "can't authenticate client: maximum attempts reached",
    ))?
}

fn try_auth(certs_lock: Arc<RwLock<Option<Vec<Certificate>>>>) -> Option<Result<Vec<u8>, Status>> {
    let certs_guard = certs_lock.read().expect("poisoned lock");

    certs_guard.as_ref().map(|certs| {
        ...
        certs
            ...
            .and_then(|client_cert| {
                X509::from_der(client_cert.as_ref())
                    ...
                    .map_err(|_| {
                        Status::internal(
                            "can't authenticate client: unable to parse cert fingerprint",
                        )
                    })
            })
    })
